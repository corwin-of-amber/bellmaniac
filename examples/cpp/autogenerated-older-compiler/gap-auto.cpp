...
#include "preface.h"
#include "input.h"
#define FOR_C_loop_1 FOR_FWD
#define FOR_C_loop_2 FOR_FWD
#define FOR_C_loop_3 FOR_FWD

#define FOR_C_rec_1 FOR_FWD
#define FOR_C_rec_2 FOR_FWD
#define FOR_C_rec_3 FOR_FWD
#define FOR_C_rec_4 FOR_FWD


#define FOR_B_loop_1 FOR_FWD
#define FOR_B_loop_2 FOR_FWD
#define FOR_B_loop_3 FOR_FWD

#define FOR_B_rec_1 FOR_FWD
#define FOR_B_rec_2 FOR_FWD
#define FOR_B_rec_3 FOR_FWD
#define FOR_B_rec_4 FOR_FWD

#define NOTLETSTMT 1

#define FOR_A_loop_1 FOR_FWD
#define FOR_A_loop_2 FOR_FWD
#define FOR_A_loop_3 FOR_FWD
#define FOR_A_loop_4 FOR_FWD
#define FOR_A_rec_1 FOR_FWD
#define FOR_A_rec_2 FOR_FWD
#define FOR_A_rec_3 FOR_FWD
#define FOR_A_rec_4 FOR_FWD


#define INSET(a,b) In(PARAM(b),a)

#define DCLdist(i,j) psi(i,j)
#define DBLdist(i,j) psi(i,j)
#define DALdist(i,j) psi(i,j)

#define BASE_CONSTRAINT_C(a,b,c) BASE_CONSTRAINT(PARAM(a),PARAM(b),PARAM(c))
#define BASE_CONSTRAINT_B BASE_CONSTRAINT_C
#define BASE_CONSTRAINT_A(a,b) BASE_CONSTRAINT(PARAM(a),PARAM(b))
	...
	FOR_C_loop_2(i,DEFBEGIN(J1),DEFEND(J1)){
		FOR_C_loop_3(j,DEFBEGIN(K),DEFEND(K)){
			...
			FOR_C_loop_1(p,DEFBEGIN(J0),DEFEND(J0)){
				t22 = min(t22,psiCopyOpt(p,j,J0,K)+w_prime(p,i)/* INSET(p,J0) */);
			}
...
#if NOTLETSTMT
	FOR_C_rec_1(i,max(DEFBEGIN(L2),DEFBEGIN(L2)+1),DEFEND(L2)){
		FOR_C_rec_2(j,max(DEFBEGIN(K1),DEFBEGIN(K1)+1),DEFEND(K1)){
...
void funcC_2(DEFINTERVALFUNC(K1),DEFINTERVALFUNC(K0),DEFINTERVALFUNC(J0),DEFINTERVALFUNC(L3)){
	FOR_C_rec_3(i,max(DEFBEGIN(L3),DEFBEGIN(L3)+1),DEFEND(L3)){
		FOR_C_rec_4(j,max(DEFBEGIN(K1),DEFBEGIN(K1)+1),DEFEND(K1)){
...
void funcC_rec(DEFINTERVALFUNC(J0),DEFINTERVALFUNC(J1),DEFINTERVALFUNC(K)){
	...
	/* PARLEVEL 0 */ funcC_2(PARAM(K1),PARAM(K0),PARAM(J0),PARAM(L3)); /*function doesn't do anything */
	cilk_sync;
...
#if NOTLETSTMT
	FOR_B_rec_1(i,max(DEFBEGIN(J1),DEFBEGIN(J1)+1),DEFEND(J1)){
		FOR_B_rec_2(j,max(DEFBEGIN(M2),DEFBEGIN(M2)+1),DEFEND(M2)){
...
void funcB_2(DEFINTERVALFUNC(K0),DEFINTERVALFUNC(J0),DEFINTERVALFUNC(J1),DEFINTERVALFUNC(M3)){
	FOR_B_rec_3(i,max(DEFBEGIN(J1),DEFBEGIN(J1)+1),DEFEND(J1)){
		FOR_B_rec_4(j,max(DEFBEGIN(M3),DEFBEGIN(M3)+1),DEFEND(M3)){
...
void funcB_rec(DEFINTERVALFUNC(J),DEFINTERVALFUNC(K0),DEFINTERVALFUNC(K1)){
	...
	/* PARLEVEL 0 */ funcB_2(PARAM(K0),PARAM(J0),PARAM(J1),PARAM(M3)); /*function doesn't do anything */
	cilk_sync;
...
#if NOTLETSTMT
	FOR_A_rec_1(i,max(DEFBEGIN(J1),DEFBEGIN(J1)+1),DEFEND(J1)){
		FOR_A_rec_2(j,max(DEFBEGIN(K1),DEFBEGIN(K1)+1),DEFEND(K1)){
