...
void func(DEFINTERVALFUNC(J0),DEFINTERVALFUNC(K0),DEFINTERVALFUNC(K2),DEFINTERVALFUNC(K3)){
...
void func0(DEFINTERVALFUNC(J0),DEFINTERVALFUNC(K1),DEFINTERVALFUNC(K2),DEFINTERVALFUNC(K3)){
	FOR_FWD(i,DEFBEGIN(K2),DEFEND(K2)){
		FOR_FWD(j,max((i + 1),DEFBEGIN(K3)),DEFEND(K3)){
			TYPE tmp7;
			tmp7 = INITMIN;
...
void funcC_rec(DEFINTERVALFUNC(J0),DEFINTERVALFUNC(J1)){
	if(BASE_CONSTRAINT(PARAM(J0),PARAM(J1))){
		...
		cilk_spawn func(PARAM(J0),PARAM(K0),PARAM(K2),PARAM(K3));;
		funcC_rec(PARAM(K0),PARAM(K3));
		cilk_sync;

		cilk_spawn funcC_rec(PARAM(K1),PARAM(K2));;
		cilk_spawn func0(PARAM(J0),PARAM(K1),PARAM(K2),PARAM(K3));;
...
void func3(DEFINTERVALFUNC(J),DEFINTERVALFUNC(J0),DEFINTERVALFUNC(J1)){
	FOR_FWD(i,DEFBEGIN(J0),DEFEND(J0)){
		FOR_FWD(j,max((i + 1),DEFBEGIN(J1)),DEFEND(J1)){
			TYPE tmp12;
			tmp12 = INITMIN;
...
void funcA_rec(DEFINTERVALFUNC(J)){
	if(BASE_CONSTRAINT(PARAM(J))){
		...
		funcA_rec(PARAM(J0));
		func3(PARAM(J),PARAM(J0),PARAM(J1));
