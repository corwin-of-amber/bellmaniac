...
respectively~(\Cref{overview:quadrants-abstract}).
Richard obtains the program in \Cref{intro:breakdown}, which he names ``A'' 
...
is sound.
A stratified computation can be seen in \Cref{overview:chain}. This does not,
...
written by the first loop, as shown in \Cref{intro:further-breakdown}.
...
Richard finally has the version in \Cref{intro:recursive-A}.
...
Richard and other algorithm designers to produce a {\bf provably correct}
implementation of this algorithm through a series of high-level tactic
application.
