...
implementation (AUTO) on the standard parallelized loop implementation 
(LOOPDP)and it compares AUTO with state-of-the-art parallelized 
implementations (CO and COZ) from~\cite{IPDPS15/Tithi}. $N$ is the problem size and 
$B$ is the base case size for using loops instead of recursion. It can be seen from the 
table that our implementation performs close to the manually optimized code. 
Note that there are some low-level and domain specific optimizations in CO and COZ 
like using pointer arithmetics for loop traversals, using a z-morton layout of 
the array etc which are out of scope of this work.
...
\begin{tabular}{|l|c|c|c|}
  ...
  %\hline
\end{tabular}
\caption{\label{evaluation:cpp runtimes}
