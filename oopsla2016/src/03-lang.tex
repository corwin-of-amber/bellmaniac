...
we first set up a small language that can easily express our specifications,
including intermediate ones, and can also be translated into executable
code in a straightforward fashion. We choose a functional language
setting, where scalar types are array indices ($i, j, k : J$ in the example)
and values stored in the arrays, typically real numbers ($G_{ij} : \R$ in the example).
Arrays are then encoded as functions, or arrow types, mapping index
values to real values (e.g. $G : J^2\to\R$). We want to have a notion of
...
(two or more), which define predicates $J_0,J_1 : J\to\B$ (where $\B$ is the Boolean type)
and the refined types $\widehat{J}_0=\{v\,{:}\,J \,|\, J_0(v)\}$
and $\widehat{J}_1=\{v\,{:}\,J \,|\, J_1(v)\}$, which become subtypes of G.
Usually, we can omit the ``hat'' and use $J_0$ as a synonym for $\widehat{J}_0$ 
when its clear from the context that it designates a type. 
...
of the ternary operator (known as {\tt ?:} or {\tt ite}) to functions,
...
\coa{More formal discussion starts here}

We set up a formal language based on the polymorphic
...
to properly emit memory access and parallelization primitives.
...
\[
  ...
  \begin{array}{@{}l@{}l@{}l@{}}
    \lspan2{x : J \to \R} \\
    \lspan2{w : J^3 \to \R} \\
    G ~=~ \fix~(\theta:J^2\,{\cap}\,{<}\to\R)\,i\,j \mapsto{}
...
Note that the range for $\min k$ is implicit, given the type of
$\theta$: 
