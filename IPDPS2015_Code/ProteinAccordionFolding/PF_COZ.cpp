...
/*
 ...
 SOF  = single one fold score.
 S_dp = protein accordion folding score table fot iterative implementation
 S = protein accordion folding score table fot recursive implementation
 F = folding score for recursive
 Z = Z morton conversion of folding score.
 proteinSeq = protein sequence for which we are trying to get the score for
...
/*Function to convert a row major matrix to Z-morton row major matrix. SOF is the row major, x is Z-morton row major, ix, jx are the indices of the top left corner. ilen and jlen are the lengths of the x and y dimensions.
 */
...
//Computes score one fold function. Please see the papert and pseudocode for more explanation.
...
/*
 Parallel iterative algorithm that solves protein accordion folding problem.
 */
...
/*
 Function D for the recursive Algo. This implementation works for any N. Here x is the output matrix, v, DN, sof and sofdn are the input matrices.
 here sof is the corresponding score one fold score for v and sofdn is the corresponding score one fold score for DN.
 xi, xj = index of the top-left corner of output matrix, x.
 xi, vj = index of the top-left corner of input matrix, v.
 xilen = length of row dimension for x.
 xjlen = length of column dimension for x
 vilen = length of row dimension for v.
 vjlen = length of column dimension for v.
 vdnilen = length of column dimension for DN.
...
void D_PF(int *x, int*v, int *DN, int *sof, int *sofdn, int xi, int xj, int vj,
    ...
    if (xilen <= B && xjlen <= B && vjlen <= B) //D_loop
    {
        ...
        if ( (retval=PAPI_start(EventSet[id])) != PAPI_OK)
            ERROR_RETURN(retval);
#endif
        // FInds the actual end with valid data
        int endi = (xi + xilen >= N) ? (N - xi - 1) : xilen;
        int endj = (xj + xjlen >= N) ? (N - xj - 2) : xjlen;
        int endk = (vj + vjlen >= N) ? (N - vj) : vjlen;
        int *vv;
        // The iterative basecase. Please draw the diagrams first to understand why
...
//void D_PF(int *x, int*v, int *DN, int *sof, int *sofdn, int xi, int xj, int vj, int xilen, int xjlen, int vilen, int vjlen, int vdnilen)
/*
 Function C for the recursive Algo. This implementation works for any N. Here x is the output matrix, u, DN, sof and sofdn are the input matrices.
 here sof is the corresponding score one fold score for v and sofdn is the corresponding score one fold score for DN.
 xi, xj = index of the top-left corner of output matrix, x.
 xi, uj = index of the top-left corner of input matrix, u.
 xilen = length of row dimension for x.
 xjlen = length of column dimension for x
 ujlen = length of column dimension for v.
 udnilen = length of column dimension for DN.
 Pointers arithmatics are used to improve running time all around.
 */
void C_PF(int *x, int *u, int *DN, int *sof, int *sofdn, int xi, int xj, int uj,
    ...
    if (xilen <= B && xjlen <= B && ujlen <= B) //C_loop
    {
        ...
        // Finds the actual ends with valid data
        int endi = (xi + xilen >= N) ? (N - xi - 1) : xilen;
        int endj = (xj + xjlen >= N) ? (N - xj - 2) : xjlen;
...
//For function B, we do not need the down pointer
/*
 Function B for the recursive Algo. This implementation works for any N. Here x is the output matrix, v, and sof are the input matrices.
 here sof is the corresponding score one fold score for v.
 xi, xj = index of the top-left corner of output matrix, x.
 xi, vj = index of the top-left corner of input matrix, u.
 xilen = length of row dimension for x.
 xjlen = length of column dimension for x
 vilen = length of row dimension for v.
 Pointers arithmatics are used to improve running time all around.
...
void B_PF(int *x, int *v, int *sof, int xi, int xj, int vj, int xilen,
          int xjlen, int vilen) {
    // If indices are outside the actual data range, return.
    if (xi > N || xj > N || vj > N)
        return;
...
/*
 Function A for the recursive Algo. This implementation works for any N. Here x is the output matrix, v, DN, sof and sofDN are the input matrices.
 here sof is the corresponding score one fold score for v.
 xi, xj = index of the top-left corner of output matrix, x.
 xilen = length of row dimension for x.
 xjlen = length of column dimension for x
 xdnilen = length of column dimension for DN.
 Pointers arithmatics are used to improve running time all around.
 */
...
// Recursive divide and conquer algorithm for protein accordion folding
int CO_PROTEIN_FOLDING(int n) {
    
    //array, starting point x, starting point y, size,
    // S = protein accordion folding score
    // F = score one fold score
    A_PF(S, S, F, F, 0, 0, n, n, n);
    
    // Convert the result back from Z morton to row major order
...
int main(int argc, char ** argv) {
...
#ifdef CO
    F = ( int *) _mm_malloc( N * N * sizeof( int ), 64); //stores score-one-fold for recursive
#endif
    ...
    // Covert score one fold to Z morton layout.
    conv_RM_2_ZM_RM(F, 0, 0, N, N);
    cout<<"CO:";
    
    // Temorary storage to hold Z morton score of protein accordion folding
