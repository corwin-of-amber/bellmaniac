...
While the former contains many techniques that make heavy use of solvers\marginpar{Sketch, Leon\ldots},
The latter relies mostly on theorem proof assistants such as Isabelle/HOL and Coq.
In a framework developed by Manna and Waldinger \marginpar{(e.g. 1980, 1992)}, the specification
is stated as a logical goal to be proven. The program is than read by interpreting the proof via the Curry-Howard
correspondance (also known as ``proofs-as-programs''). \newterm{Transformational} approaches\marginpar{KIDS/Specware, Spiral, AutoBayes, and also Fiat},
...
\begin{itemize}
  \item The solver can directly check that a program term is requivalent to another term,
...
at same $t=a+c$ is given by $\yw{abc}$. \coa{I changed $i$,$j$,$p$,$q$ to $a$,$b$,$c$ to avoid confusion with what comes later on.}
